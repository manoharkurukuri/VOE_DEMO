...
class VehicleIncentiveLLM(BaseModel):
    ...
    offer_priority: str | None = Field(
        default=None,
        description="Website order label. Backend overwrites this as Vehicle #1, #2, ...",
    )
...
class OfferExtractionResponse(BaseModel):
    ...
    offers: list[VehicleIncentiveLLM] = Field(
        ...
        description=(
            "Every distinct vehicle offer on the page, in the same top-to-bottom "
            "order as the website. Do not cap or limit the number of offers."
        ),
    )
